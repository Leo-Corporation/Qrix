      ...
      switch (tab) {
        case 'email':
          textContent = `mailto:${mail}?subject=${subject.replaceAll(
            ' ',
            '%20',
          )}&body=${message.replaceAll(' ', '%20').replaceAll('\n', '%0A')}`;
          break;
        case 'sms':
      ...
      <section className="flex w-full flex-col items-center">
        <div className="m-5 flex w-full space-x-2">
          <Tabs defaultValue="text" className="w-full">
            <TabsList className="my-2 flex h-auto w-full flex-col space-y-2 sm:my-0 sm:w-fit sm:flex-row sm:space-y-0">
              <span>
        ...
        <canvas className={vis ? 'max-w-full' : 'hidden'} id="qrcode"></canvas>
